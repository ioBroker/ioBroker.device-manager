...
class Communication<P extends CommunicationProps, S extends CommunicationState> extends Component<P, S> {
    ...
    // eslint-disable-next-line react/no-unused-class-component-methods
    loadDevices(): Promise<DeviceInfo[] | null> {
        if (!this.state.selectedInstance) {
            throw new Error('No instance selected');
        }
        ...
        return new Promise(resolve => {
            ...
            this.props.socket
                .sendTo(this.state.selectedInstance, 'dm:listDevices')
                .then(result => {
    ...
    // eslint-disable-next-line react/no-unused-class-component-methods
    loadInstanceInfos(): Promise<InstanceDetails | null> {
        if (!this.state.selectedInstance) {
            throw new Error('No instance selected');
        }
        return new Promise(resolve => {
            ...
            this.props.socket
                .sendTo(this.state.selectedInstance, 'dm:instanceInfo')
                .then(result => {
                    this.responseTimeout && clearTimeout(this.responseTimeout);
                    this.responseTimeout = null;
                    resolve(result);
