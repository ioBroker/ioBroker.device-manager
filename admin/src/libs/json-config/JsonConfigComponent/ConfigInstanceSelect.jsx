...
class ConfigInstanceSelect extends ConfigGeneric {
	async componentDidMount() {
		...
		this.props.socket.getAdapterInstances(adapter, true).then(async (instances) => {
			let selectOptions;
			if (this.props.schema.adapter === '_dataSources') {
				...
				instances = instances.filter(
					(instance) =>
						instance && instance._id.startsWith('system.adapter.' + this.props.schema.adapter + '.'),
				);
			}
	...
	componentWillUnmount() {
		this.props.socket.unsubscribeObject('system.adapter.*', this.onInstancesUpdate).then(() => {});
	...
	renderItem(error, disabled, defaultValue) {
		...
		return (
			<FormControl className="" key={this.props.attr} variant="standard">
