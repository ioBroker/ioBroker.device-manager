...
class ConfigTable extends ConfigGeneric {
		...
		return (
			<TableHead>
				<TableRow>
								...
								<div className={classes.flex}>
									...
									{headCell.filter && this.state.filterOn.includes(headCell.attr) ? (
										<TextField
											...
											InputProps={{
												endAdornment: this.filterRefs[headCell.attr]?.current?.children[0]
													?.children[0]?.value && (
										...
										<span className="">{this.getText(headCell.title)}</span>
									)}
									{headCell.filter ? (
										<IconButton
											...
											onClick={() => {
												...
												this.setState({ filterOn }, () => {
													if (
														pos &&
														this.filterRefs[headCell.attr].current.children[0].children[0]
															.value
													) {
														this.filterRefs[
															headCell.attr
					...
					{!schema.noDelete && (
						<TableCell
							style={{ paddingLeft: 20, paddingRight: 20, width: buttonsWidth, textAlign: 'right' }}
							padding="checkbox"
						>
	...
	isAnyFilterSet() {
		return Object.keys(this.filterRefs).find(
			(attr) => this.filterRefs[attr].current?.children[0].children[0].value,
		);
	}
		...
		return (
			<Paper className={classes.paper}>
				<div className={classes.addIcon}>
					{schema.label ? (
						<Toolbar variant="dense" className={classes.rootTool}>
									...
									{!schema.noDelete && (
										<TableCell align="left" className={classes.buttonCell}>
											{!doAnyFilterSet && !this.state.orderBy ? (
												i ? (
													<Tooltip title={I18n.t('ra_Move up')}>
											...
											{!doAnyFilterSet && !this.state.orderBy ? (
												i < visibleValue.length - 1 ? (
													<Tooltip title={I18n.t('ra_Move down')}>
											...
											{this.props.schema.clone ? (
												<Tooltip title={I18n.t('ra_Clone current row')}>
					...
					{!visibleValue.length && value.length ? (
						<div className={classes.filteredOut}>
							<Typography className={classes.title} variant="h6" id="tableTitle" component="div">
								...
								<IconButton size="small" onClick={(e) => this.applyFilter(true)}>
									<CloseIcon />
								</IconButton>
							</Typography>
						</div>
