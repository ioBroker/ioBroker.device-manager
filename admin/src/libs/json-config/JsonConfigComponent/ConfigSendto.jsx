...
class ConfigSendto extends ConfigGeneric {
	...
	_onClick() {
		...
		const _origin = `${window.location.protocol}//${window.location.host}${window.location.pathname.replace(
			/\/index\.html$/,
			'',
		)}`;
		const _originIp = `${window.location.protocol}//${
			this.state.hostname.split(':').length > 3 ? `[${this.state.hostname}]` : this.state.hostname
			...
			.then((response) => {
				...
				if (response?.error) {
					...
					if (response?.native && this.props.schema.useNative) {
						...
						attrs.forEach((attr) => this.onChange(attr, response.native[attr]));
