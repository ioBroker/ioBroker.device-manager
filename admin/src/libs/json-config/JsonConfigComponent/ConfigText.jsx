		...
		if (isIndeterminate) {
			...
			return (
				<Autocomplete
					...
					onChange={(_, value) => this.onChange(this.props.attr, value ? value.value : '')}
					options={arr}
				...
				return (
					<div style={{ width: '100%' }}>
						{this.props.schema.label ? (
							<div className="">{this.getText(this.props.schema.label)}</div>
						) : null}
						<TextareaAutosize
							variant="standard"
							style={{
							...
							placeholder={this.getText(this.props.schema.placeholder)}
						/>
						{helper || error ? <div className="">{error || helper}</div> : null}
					</div>
				);
			} else {
				return (
