	...
	renderItem(error, disabled, defaultValue) {
		...
		const password = (
			<TextField
				...
				InputProps={{
					endAdornment:
						this.state.value && this.state.value !== PASSWORD_PLACEHOLDER && this.props.schema.visible ? (
		...
		if (this.props.schema.repeat) {
			const passwordRepeat = (
				<TextField
					...
					InputProps={{
						endAdornment:
							this.state._repeat && this.state._repeat !== PASSWORD_PLACEHOLDER ? (
								<InputAdornment position="end">
									<IconButton
										size="large"
										tabIndex={-1}
			...
			return (
				<div className="">
					...
					<div className="">{passwordRepeat}</div>
				</div>
			);
		} else {
			return password;
