		...
		if (this.state.showLicenseData) {
			...
			Object.keys(data).forEach((key) => {
				...
				if (typeof data[key] === 'object') {
					...
					pre.push(
						<div key={key}>
							<div className="">{key.replace(/_/g, ' ')}:</div>
							{data[key]}
						</div>,
					);
				}
			...
			pre.push(
				<div key="checked">
					<div className="">{I18n.t('ra_Checked')}:</div>
			...
			return (
				<Dialog open={!0} onClose={() => this.setState({ showLicenseData: null })}>
					<DialogTitle>
						<span className="">{I18n.t('ra_License %s', this.state.result ? 'OK' : 'INVALID')}</span>
					</DialogTitle>
					<DialogContent>
						{this.state._error ? <div className="">{this.state._error}</div> : null}
						{pre}
					</DialogContent>
					<DialogActions>
						<Button
