	...
	onDrop(acceptedFiles) {
		...
		reader.onload = () => {
			...
			if (file.size > maxSize) {
				return window.alert(
					I18n.t('File is too big. Max %sk allowed. Try use SVG.', Math.round(maxSize / 1024)),
				);
			}
	...
	renderItem(error, disabled, defaultValue) {
		...
		const element = (
			<div className="">
				...
				{play && (
					<IconButton style={{ color: '#00FF00' }} onClick={() => this.play()}>
						<IconPlay />
					</IconButton>
				)}
			</div>
			...
			return (
				...
				>
					{({ getRootProps, getInputProps }) => (
						<div className={Utils.clsx(this.state.uploadFile === 'dragging')} {...getRootProps()}>
							<input {...getInputProps()} />
							{this.state.uploadFile === 'dragging' ? (
								<div className={Utils.clsx(this.state.uploadError)}>
									<div className="">
